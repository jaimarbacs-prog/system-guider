...
return next()`),
        ...
        h1('8. Scoring engine'),
        p(
          'During recording, System Guider stores a CSS selector plus optional match hints captured from the target (label text, href, section, id, and similar). During playback, candidates are ranked by scoreElement(match).',
        ),
        note(
          'data-guider is optional. Guides work without it: the library still records match hints and uses scoring to find the best element. Adding data-guider is recommended for maximum stability when labels or layout change.',
        ),
        h2('8.1 Hint fields'),
        bullet('dataGuider — optional stable hook via data-guider="…" (highest weight when present)'),
        bullet('Always useful without hooks: text, href, section, id, name, placeholder, ariaLabel, role, tag, type'),
        h2('8.2 Weight summary'),
        table(
          ['Signal', 'Typical weight'],
          [
            ['data-guider exact', '+100 (wrong hook −40) — only if you added the attribute'],
            ['id exact', '+80'],
        ...
        spacer(),
        h2('8.3 With or without data-guider'),
        p(
          'Default path (no markup changes): record the flow as usual. Playback uses the saved selector first; if it fails or is weak, scoring compares candidates using the recorded hints (text, href, section, and so on).',
        ),
        p(
          'Hardened path (optional): mark critical controls so scoring prefers an exact hook even when copy or DOM structure changes:',
        ),
        ...codeBlock(`<!-- Optional — not required for recording/playback -->
